...
//esse middleware so vao aceitar o json
...
app.get('/criartabelausuarios', (req, res) => {
    pool.connect((err, client) => {
        ...
        var sql = 'CREATE TABLE usuarios (id numeric, nome varchar(100), email varchar(50), senha varchar(200), dataNascimento varchar(10), gestante varchar, pcd varchar, perfil varchar(15))'
...
app.delete('/usuarios/:email', (req, res) => {
    pool.connect((err, client) => {
        ...
        client.query('DELETE FROM usuarios WHERE email = $1 ', [req.params.email], (error, result) => {
            if(error) {
                return res.status(401).send('Operacao nao autorizada')  
            }
